...
final class ElasticSearchViewModel: ObservableObject{
    ...
    func reverseLocation(address: String) {
        NetworkService.shared.sendRequest(
            ...
            params: ["text": address,
                     "boundary.circle.lat": String(Constants.latitude),
                     "boundary.circle.lon": String(Constants.longitude),
                     "boundary.circle.radius": "15",
                     "focus.point.lat": String(Constants.latitude),
                     "focus.point.lon": String(Constants.longitude),
